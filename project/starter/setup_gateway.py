...
def find_by_name(list_fn, name, **kwargs):
    """Find a named resource while following paginated list responses."""
    next_token = None
    while True:
        request = dict(kwargs)
        if next_token:
...
def main():
    ...
    parser.add_argument("--region", default=DEFAULT_REGION,
                        help="AWS region containing the CloudFormation stack.")
    ...
    acc = boto3.client("bedrock-agentcore-control", region_name=args.region)

    gateway = find_by_name(acc.list_gateways, gateway_name)
    if gateway:
    ...
    gateway_details = acc.get_gateway(gatewayIdentifier=gateway_id)
    ...
    target = find_by_name(
        acc.list_gateway_targets,
        args.target_name,
        gatewayIdentifier=gateway_id,
    )
    if target:
        print(f"Gateway target '{args.target_name}' already exists; reusing it.")
    else:
